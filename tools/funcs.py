...
def explain_c_declaration(c_decl, expand_struct=True, expand_typedef=True, filename='<stdin>'):
    ...
    try:
        ...
        exp_list = []
        for ext in node.ext:
            expanded = expand_struct_typedef(ext, node,
    ...
    return [_explain_decl_node(e) for e in node.ext]
...
def _explain_decl_node(decl_node):
    ...
    if type(decl_node.type) == c_ast.FuncDecl:
        fname = decl_node.name
        fargs = _explain_type(decl_node.type)
        ftype = _explain_type(decl_node.type.type)
        return '"%s": {"ret": "%s", "args": [%s]},' % (fname, ftype, fargs)

    try:
        return (decl_node.name + " is a " + storage +
...
def _explain_type(decl):
    ...
    if typ == c_ast.TypeDecl:
        ...
        return 'int'
        #return ('enum %s' % (decl.name if decl.name else ''))
    ...
    if typ in (c_ast.Decl, c_ast.TypeDecl, c_ast.PtrDecl, c_ast.ArrayDecl):
        ...
        if decl.args:
            for param in decl.args.params:
                param.type = _expand_in_place(param.type, file_ast, expand_struct,
                                              expand_typedef)
        ...
        if not expand_struct:
          ...
          decl.names[0] not in ('int', 'char', 'float', 'double', 'long', 'void')):
...
if __name__ == "__main__":
    ...
    print("\n".join(explain_c_declaration(c_decl, filename=filename)))
